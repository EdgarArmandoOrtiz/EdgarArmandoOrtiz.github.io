export class CanvasLocal {
  ...
  drawHexface(x1: number, y1: number, width: number, height: number, r: number, g: number, b: number): void{
   ...
   drawHexface2(x1: number, y1: number, width: number, height: number, r: number, g: number, b: number): void{
    let unidad = width;
  ...
  paint():void {
   ...
   // let colorRelleno: number[]=
    ...
    let ind = 0;
      ...
      let h1: number[] = [1.8,5.1,4,7];
      let r: number[] = [229,255,57,255];
      let g: number[] = [9,0,255,255];
      let b: number[] = [127,0,20,0];
  for (let i = 1; i <= 8; i += 2) {
        
      this.drawHexface(this.iX(i), this.iY(4 * h[ind] / maxEsc), this.iX(1) - this.iX(0),
        this.iY(4 * h[ind] / maxEsc) - this.iY(0), r[ind], g[ind], b[ind],);

        this.drawHexface2(this.iX(i), this.iY(4 * h[ind] /maxEsc), this.iX(1) - this.iX(0),
        this.iY( h1[ind] * h[ind] / maxEsc) - this.iY(0), r[ind], g[ind], b[ind]);
        ind++; 
    }
    ind=0
    for (let x = 0; x < 8; x += 2) {
